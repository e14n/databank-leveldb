...
LevelDBDatabank.prototype.connect = function(params, callback) {
    ...
    leveldb.open(bank.file, { create_if_missing: true }, function(err, db) {
        if (err) {
            callback(err);
        } else {
            bank.db = db;
...
LevelDBDatabank.prototype.create = function(type, id, value, callback) {
    ...
    bank.db.put(key, JSON.stringify(value), function(err) {
        if (err) {
            callback(err, null);
        } else {
            callback(null, value);
...
LevelDBDatabank.prototype.read = function(type, id, callback) {
    ...
    bank.db.get(key, function(err, value) {
        if (err) {
            callback(err, null);
        } else {
